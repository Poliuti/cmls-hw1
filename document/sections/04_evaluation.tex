...
At first, we computed the scores over a simple \texttt{LinearRegression} to evaluate the previous part of feature selection (see section~\ref{sec:dataset-??}).
Then we took advantage of the grid search to find the suitable parameters for each $\nu$-SVM, $\epsilon$-SVM and K-neighbors-regressor for each annotation to be predicted (see section~\ref{sec:regression-??}).
...
We then extracted the metrics of the best estimators (see table~\ref{table:cross-scores}). It can be observed that, even after several attempts of feature selection (both manual and automatic), and optimization through cross-validation, no regressor could obtain a significant positive $R^2$-score for the values of ``valence std'' and ``arousal std'', and the obtained $R^2$-scores for ``valence mean'' and ``arousal mean'' are still quite low. This might be a consequence of the quality of the provided dataset or some preprocessing step on the dataset that we didn't manage to accomplish.
...
In particular, given the results in table~\ref{table:cross-scores}, we decided to use Ridge, $\nu$-SVM and KN-regression as candidates for the final testing stage, as they seemed to be the best scoring regressors for each family of regression models.
